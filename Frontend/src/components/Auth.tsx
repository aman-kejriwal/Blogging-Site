...
export const Auth = ({ type }: { type: 'signin' | 'signup' }) => {
    ...
    return (
        <>
            {
                issignupopen && (
                    <SignupDialog onClose={() => setissignupopen(false)} type={type} />
                )
            }
            <div className="flex justify-center items-center min-h-screen bg-surface-50 px-6">
                <div className="w-full max-w-md animate-fade-in-up" style={{ animationFillMode: 'both' }}>
                    ...
                    {/* <button className="w-full" onClick={() => {
                        ...
                        <p className=" hover:text-black transition-colors duration-200 text-center text-sm text-ink-50 font-normal mt-4">
                            More {type === 'signin' ? 'sign in' : 'sign up'} options
                        </p>
                    </button> */}
